...
public final class BpmnJobBehavior {
  ...
  private Either<Failure, JobProperties> evaluateJobExpressions(
    ...
    return Either.<Failure, JobProperties>right(new JobProperties())
        ...
        .flatMap(p -> evalDueDateExp(jobWorkerProps, scopeKey).map(p::dueDate))
        .flatMap(p -> evalFollowUpDateExp(jobWorkerProps, scopeKey).map(p::followUpDate));
  }
  ...
  private Either<Failure, String> evalDueDateExp(
      final JobWorkerProperties jobWorkerProperties, final long scopeKey) {
    final Expression dueDate = jobWorkerProperties.getDueDate();
    if (dueDate == null) {
      return Either.right(null);
    }
    return expressionBehavior
        .evaluateDateTimeExpression(dueDate, scopeKey)
        .map(ZonedDateTime::toString);
  }

  private Either<Failure, String> evalFollowUpDateExp(
      final JobWorkerProperties jobWorkerProperties, final long scopeKey) {
    final Expression followUpDate = jobWorkerProperties.getFollowUpDate();
    if (followUpDate == null) {
      return Either.right(null);
    }
    return expressionBehavior
        .evaluateDateTimeExpression(followUpDate, scopeKey)
        .map(ZonedDateTime::toString);
  }
